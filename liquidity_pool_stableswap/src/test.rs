...
fn configure_rewards(
    ...
    liq_pool.set_rewards_config(
        admin,
        &env.ledger().timestamp().saturating_add(expires_in_secs),
        &reward_tps,
    );
}
...
fn test_events_2_tokens() {
    ...
    let events = e.events().all().filter_by_contract(&liqpool.address);
    assert_eq!(
        events,
        vec![
            &e,
...
fn test_rewards_state_opt_out_tracks_excluded_shares_on_balance_change() {
    ...
    let user1 = Address::generate(&env);
    let user2 = Address::generate(&env);
    let share_token = setup.token_share;
...
#[test]
fn test_estimate_deposit_and_working_balance() {
    let setup = Setup::default();
    let env = setup.env;
    let liq_pool = setup.liq_pool;
    ...
    let token1_admin = get_token_admin_client(&env, &setup.token1.address);
    let token2_admin = get_token_admin_client(&env, &setup.token2.address);
    let boost_admin = get_token_admin_client(&env, &setup.reward_boost_token.address);

    for user in [&user1, &user2] {
        token1_admin.mint(user, &1_000_0000000);
        token2_admin.mint(user, &1_000_0000000);
    }
    ...
    liq_pool.deposit(&user2, &Vec::from_array(&env, [20_0000000, 20_0000000]), &0);
    ...
    let desired_amounts = Vec::from_array(&env, [50_0000000, 50_0000000]);
    let estimated_shares = liq_pool.estimate_deposit(&desired_amounts);
    let estimated_total_user_shares = user2_shares + estimated_shares;
    ...
    let (_, minted_shares) = liq_pool.deposit(&user2, &desired_amounts, &0);
    assert_eq!(estimated_shares, minted_shares);
    assert_eq!(
