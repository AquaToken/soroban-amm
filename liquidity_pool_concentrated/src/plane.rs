...
// Since the chunk is already loaded during the search, liquidity_net is returned
// directly to avoid a redundant storage read.
pub(crate) fn find_initialized_tick(
    e: &Env,
    compressed: i32,
    ...
    let cursor_compressed = if lte {
        compressed
    } else {
        compressed.saturating_add(1)
    };
    let (chunk_pos, cursor_slot) = chunk_address(cursor_compressed);

    // Step 1 — partial scan of the cursor chunk, anchored at `cursor_slot` in
    // the scan direction.
    ...
    // Step 2 — nearest non-empty chunk within the same L1 word.
    let l1_word = u256_to_array(&get_chunk_bitmap_word(e, bm_word_pos));
    if let Some(adj_chunk_pos) = adjacent_chunk_in_l1_word(&l1_word, bm_word_pos, bm_bit_pos, lte) {
        if let Some(hit) =
            scan_chunk_for_init(e, cc, adj_chunk_pos, None, lte, limit_compressed, spacing)
        {
            return Some(hit);
        }
    }

    // Step 3 — skip empty L1 words via the L2 WordBitmap, then scan the nearest
    // non-empty chunk in the target L1 word.
    if let Some(target_word) = find_adjacent_chunk_bitmap_word(e, bm_word_pos, lte) {
        let target_bits = u256_to_array(&get_chunk_bitmap_word(e, target_word));
        if let Some(chunk_in_target) = nearest_chunk_in_l1_word(&target_bits, target_word, lte) {
...
// Scan a chunk for the first initialized slot whose compressed position falls
// within `limit_compressed`.
//
// `partial_boundary`:
//   - `Some(cursor_slot)`: partial scan anchored at the cursor. For lte scans
//     slots `[0..=cursor_slot]` descending; for !lte scans slots
//     `[cursor_slot..TICKS_PER_CHUNK)` ascending.
//   - `None`: full chunk scan. For lte descending, for !lte ascending.
//
// The first initialized slot encountered is always decisive: if it is in-limit
// we return it; otherwise further slots in the same chunk cannot be closer
// to the limit (they are strictly lower compressed for lte, strictly higher
// for !lte), so we bail early with None.
fn scan_chunk_for_init(
    e: &Env,
    cc: &mut ChunkCache,
    chunk_pos: i32,
    partial_boundary: Option<u32>,
    lte: bool,
    limit_compressed: i32,
    spacing: i32,
) -> Option<(i32, i128)> {
    let chunk = cc.get_chunk(e, chunk_pos)?;
    let (start, end_exclusive) = match partial_boundary {
        Some(boundary) if lte => (0u32, boundary + 1),
        Some(boundary) => (boundary, TICKS_PER_CHUNK as u32),
        None => (0u32, TICKS_PER_CHUNK as u32),
    };

    let mut visit = |s: u32| -> Option<Option<(i32, i128)>> {
    ...
    if lte {
        for s in (start..end_exclusive).rev() {
            if let Some(outcome) = visit(s) {
                return outcome;
            }
        }
    } else {
        for s in start..end_exclusive {
            if let Some(outcome) = visit(s) {
                return outcome;
            }
        }
...
// Adjacent chunk within the same L1 word, skipping the cursor's own bit.
// Returns None when the cursor sits on the boundary bit for the direction, or
// when the L1 word has no other set bit in that direction.
fn adjacent_chunk_in_l1_word(
    word: &[u8; 32],
    word_pos: i32,
    bit_pos: u32,
    lte: bool,
) -> Option<i32> {
    let from_bit = if lte && bit_pos > 0 {
        bit_pos - 1
    } else if !lte && bit_pos < 255 {
        bit_pos + 1
    } else {
        return None;
    };
    let bit = if lte {
        find_prev_set_bit(word, from_bit)?
    } else {
        find_next_set_bit(word, from_bit)?
    };
    Some((word_pos << 8) + bit as i32)
}

// The nearest chunk inside an L1 word from the perspective of a cursor in the
// neighbouring word: highest set bit for lte (scanning downward into a lower
// L1 word), lowest set bit for !lte (scanning upward into a higher L1 word).
fn nearest_chunk_in_l1_word(word: &[u8; 32], word_pos: i32, lte: bool) -> Option<i32> {
    let bit = if lte {
        find_prev_set_bit(word, 255)?
    } else {
        find_next_set_bit(word, 0)?
    };
    Some((word_pos << 8) + bit as i32)
}
